...
int main(int argc, char *argv[])
{
	...
	#if 0
	inet_pton(AF_INET, "224.1.2.3", &(mre.imr_multiaddr));
	inet_pton(AF_INET, argv[1], &(mre.imr_address));
	#else
	mre.imr_multiaddr.s_addr = inet_addr("224.1.2.3");
	mre.imr_address.s_addr = inet_addr(argv[1]);
	#endif
	mre.imr_ifindex = 0;
	//添加用户到多播组中
	ret = setsockopt(sd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mre, sizeof(mre));
	//帮定
